...
is easy to write clean and maintainable business code.<!--more--> 
...
* The `PersonId` class is a [value object]({{< ref "docs/business/model/value-objects.md" >}}) that is used as an identifier. 
* The `Person` class is an [entity]({{< ref "docs/business/model/entities.md" >}}) that act as the root of the `person` aggregate.
* A `changeName()` business-meaningful method is preferred over setters.
* A package materializes the boundary of the `person` [aggregate]({{< ref "docs/business/model/aggregates.md" >}}).
